...
class CartSummery
{
    function calculateCartTotals($userId, $seller_id = 0)
    {
        ...
        foreach ($cartItems as $cartItem) {
            ...
            if (isset($variant['discounted_price']) && $variant['discounted_price'] > 0) {
                $currentPrice = (int)$variant['discounted_price'];
                $subTotal += ($cartItem['quantity'] * $currentPrice);
                $discountedPricesaving += ($cartItem['quantity'] * ($variant['price'] - $currentPrice));
            } else {
            ...
            // Only add tax to total if tax is NOT included in price (exclusive tax)
            if (!empty($productTaxes) && empty($product['tax_included_in_price'])) {
                foreach ($productTaxes as $tax) {
                    $taxTotal += ($cartItem['quantity'] * $currentPrice) * $tax['percentage'] / 100;
                }
            }
        }
    ...
    function calculateCouponAmount($appliedCoupon, $subTotal, $userId)
    {
        ...
        if (!is_null($appliedCoupon)) {
            ...
            // Fetch used coupon entry for this user
            $usedCoupon = $usedCouponModel
                ->where('coupon_id', $coupon['id'])
                ->where('user_id', $userId)
                ->first();
            ...
            $canUse = false;
            if ($coupon['is_multitimes'] == 1) {
                // Multi-use coupon: always allowed
                $canUse = true;
            } else {
                // Single-use coupon: only if this user hasn't used it before
                $canUse = !$usedCoupon;
            }
            ...
            if ($canUse) {
                ...
                if ($coupon['coupon_type'] == '1') {
                    ...
                    if (!empty($coupon['max_discount_amount']) && $coupon_amount > $coupon['max_discount_amount']) {
                        $coupon_amount = $coupon['max_discount_amount'];
                    }
