...
function inv_fmt($sym, $symLeft, $value) {
    $s = '<b style="font-weight:bold;font-family:DejaVu Sans,sans-serif;">' . $sym . '</b>';
    return $symLeft ? $s . number_format((float)$value, 2) : number_format((float)$value, 2) . $s;
}

function inv_amount_in_words(float $amount): string {
    ...
    $words    = trim($rupees === 0 ? 'Zero' : rtrim($convert($rupees)));
    if ($paise > 0) {
        $words .= ' and ' . trim(rtrim($convert($paise))) . ' Paise';
    }
    return $words . ' Only';
}
...
<div id="invoice" style="font-family:'Segoe UI',Arial,sans-serif; color:#1a1a2e; background:#fff; max-width:900px; margin:0 auto;">
    ...
    <table width="100%" cellpadding="0" cellspacing="0" style="background:linear-gradient(135deg,#1a1a2e 0%,#16213e 55%,#0f3460 100%); border-radius:12px 12px 0 0; padding:0; margin:0;">
        <tr>
            ...
            <td style="padding:24px 28px; text-align:right; vertical-align:middle;">
                <div style="font-size:28px; font-weight:800; color:#111111; letter-spacing:3px; text-transform:uppercase; line-height:1;">TAX <?php echo lang('website.invoice'); ?></div>
    ...
    $thStyle = "font-size:10px; font-weight:700; text-transform:capitalize; letter-spacing:0.7px; color:#fff; padding:11px 10px; text-align:left; border:none;";
            ...
            <?php if (!empty($orderProducts)): ?>
                <?php foreach ($orderProducts as $index => $product):
                    ...
                    if ($isInclusive && $totalBreakdownTax == 0 && !empty($productTaxBreakdown)) {
                        // Amounts were stored as 0 (old data) — recompute from lineTotal and stored percentages
                ...
                <tr style="background:<?= $rowBg ?>; border-bottom:1px solid #f0f2f7;">
                    ...
                    <?php foreach ($allUniqueTaxes as $tKey => $tName): ?>
                        <td style="<?= $tdStyle ?>"><?= isset($taxByKey[$tName]) ? number_format($taxByKey[$tName]['amount'], 2) : '0.00'; ?> <?= isset($taxByKey[$tName]) ? '('.number_format($taxByKey[$tName]['percentage'], 2) . '%)' : ''; ?> </td>
                    <?php endforeach; ?>
                ...
                <table width="100%" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                        <td colspan="2" style="padding:0;">
                            <table width="100%" cellpadding="0" cellspacing="0" style="background:#1a1a2e; border-radius:8px 8px 0 0;">
                                <tr>
                            ...
                            <div style="background:#f0fdf4; border:1px solid #bbf7d0; border-top:none; border-radius:0 0 8px 8px; padding:6px 14px;">
                                <span style="font-size:10px; color:#6b7280;">Amount in Words: </span><b style="font-size:10px; color:#166534; font-family:DejaVu Sans,sans-serif;"><?= inv_amount_in_words($invTotal) ?></b>
                            </div>
                        </td>
                    </tr>
                </table>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f0f0f0;">
                    ...
                    <td style="font-size:11px; font-weight:600; color:#374151; padding:8px 12px; text-align:right;"><?= inv_fmt($sym, $symLeft, $orderDetails['delivery_charge']) ?></td>
                    <?php foreach ($allChargeTaxNames as $txName): ?>
                        ...
                        <td style="font-size:11px; font-weight:600; color:#374151; padding:8px 12px; text-align:right;"><strong><?= isset($dcByName[$txName]) ? inv_fmt($sym, $symLeft, $dcByName[$txName]['tax_amount']) : inv_fmt($sym, $symLeft, 0) ?></strong></td>
                    <?php endforeach; ?>
                </tr>
                <?php endif; ?>
                <?php if (!empty($orderDetails['additional_charge']) && $orderDetails['additional_charge'] > 0): ?>
                ...
                <tr style="border-bottom:1px solid #f0f0f0;">
                    ...
                    <td style="font-size:11px; font-weight:600; color:#374151; padding:8px 12px; text-align:right;"><?= inv_fmt($sym, $symLeft, $orderDetails['additional_charge']) ?></td>
                    <?php foreach ($allChargeTaxNames as $txName): ?>
                        ...
                        <td style="font-size:11px; font-weight:600; color:#374151; padding:8px 12px; text-align:right;"><strong><?= isset($acByName[$txName]) ? inv_fmt($sym, $symLeft, $acByName[$txName]['tax_amount']) : inv_fmt($sym, $symLeft, 0) ?></strong></td>
                    <?php endforeach; ?>
                </tr>
                <?php endif; ?>
                <?php if ($invTip > 0): ?>
                ...
                <tr style="border-bottom:1px solid #f0f0f0;">
                    ...
                    <td style="font-size:11px; font-weight:600; color:#374151; padding:8px 12px; text-align:right;"><?= inv_fmt($sym, $symLeft, $invTip) ?></td>
                    <?php foreach ($allChargeTaxNames as $txName): ?>
                        ...
                        <td style="font-size:11px; font-weight:600; color:#374151; padding:8px 12px; text-align:right;"><strong><?= isset($tipByName[$txName]) ? inv_fmt($sym, $symLeft, $tipByName[$txName]['tax_amount']) : inv_fmt($sym, $symLeft, 0) ?></strong></td>
                    <?php endforeach; ?>
                </tr>
                <?php endif; ?>
            </tbody>
        ...
        <table width="100%" cellpadding="0" cellspacing="0" style="border-collapse:collapse;">
            ...
            <tbody>
                ...
                <tr style="border-bottom:1px solid #f0f0f0;">
                    ...
                    <td style="font-size:11px; font-weight:600; color:#374151; padding:7px 12px; text-align:right;"><strong><?= inv_fmt($sym, $symLeft, $cts['amount']) ?></strong></td>
                </tr>
                <?php endforeach; ?>
                <tr style="background:#f5f5f5; border-top:1px solid #e0e0e0;">
                    ...
                    <td style="font-size:11px; font-weight:700; color:#1a1a2e; padding:7px 12px; text-align:right;"><strong><?= inv_fmt($sym, $symLeft, $grandTaxTotalWeb) ?></strong></td>
                </tr>
            </tbody>
        </table>
    </div>
