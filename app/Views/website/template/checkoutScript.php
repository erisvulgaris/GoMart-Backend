<script>
        ...
        try {
            ...
            if (result.status === 'success') {
                ...
                if (!addressWithActiveStatus && addressesWithInActiveStatus.length === 0) {
                    addressDiv.innerHTML = '<p class="dark:text-gray-300">No addresses found.</p>';
                }
            } else {
    ...
    function generateAddressHTML(address, isActive) {
        ...
        const bgColor = isActive ? 'bg-[#FFF4F1] dark:bg-red-900/20 border-red-400' : 'bg-[#F7F7F7] dark:bg-gray-700 border-[#F7F7F7] dark:border-gray-600';
        ...
        return `<div class="w-full flex ${bgColor} py-2 px-2 rounded-lg cursor-pointer border mb-2 md:mb-0 address-card ${isActive ? 'active' : ''}" onclick="setActiveAddress(this, ${address.id})">
                <div class="w-4/5 pr-4" onclick="event.stopPropagation(); setActiveAddress(this.parentElement, ${address.id});">
                    <span class="text-base font-medium capitalize dark:text-white">
                        <i class="${iconClass}"></i>
                        ${address.address_type}:
                    </span>
                    <span class="text-base font-medium capitalize mb-1 dark:text-white">${address.user_name}</span>
                    <span class="block text-sm leading-6 dark:text-gray-300">${address.user_mobile}</span>
                    <span class="block text-sm leading-6 dark:text-gray-300">${address.user_email || ''}</span>
                    <span class="block text-sm leading-6 dark:text-gray-300">${address.flat},</span>
                    <span class="block text-sm leading-6 dark:text-gray-300">${address.address},</span>
                    <span class="block text-sm leading-6 dark:text-gray-300">${address.area}, ${address.city},</span>
                    <span class="block text-sm leading-6 dark:text-gray-300">${address.state},</span>
                    <span class="block text-sm leading-6 dark:text-gray-300">${address.pincode}</span>
                </div>
                ${deleteBtnShow}
            </div>`;
    }
    ...
    async function setActiveDate(button) {
        ...
        const allButtons = document.querySelectorAll('button.date');
        allButtons.forEach(btn => btn.classList.remove('active-date'));
    ...
    function appendTimeSlots(timeslots) {
        ...
        if (timeslots.length === 0) {
            parentDiv.innerHTML = '<p class="text-gray-500 dark:text-gray-400">No available time slots for the selected date.</p>';
            return;
        }
        ...
        timeslots.forEach(slot => {
            ...
            btn.className = 'flex-shrink-0 text-sm whitespace-nowrap border py-2.5 px-4 rounded-xl bg-gray-50 dark:bg-gray-700 border-gray-200 dark:border-gray-600 text-gray-700 dark:text-gray-200 hover:border-purple-400 hover:bg-purple-50 dark:hover:border-purple-400 dark:hover:bg-purple-900/20 transition-all duration-150 time';
            btn.setAttribute('onclick', 'setActiveTime(this)');
            btn.setAttribute('data-time', `${mintime} - ${maxtime}`);
            btn.textContent = `${mintime} – ${maxtime}`;
    ...
    function setActiveTime(button) {
        ...
        const allButtons = document.querySelectorAll('button.time');
        allButtons.forEach(btn => btn.classList.remove('active-time')); // Remove previously active styling

        // Add 'active' class to the clicked button
        button.classList.add('active-time'); // Add active styles
                ...
                if (result.data.length == 0) {
                    ...
                    result.data.forEach(coupon => {
                        ...
                        couponCard.innerHTML = `
                            <div class="flex flex-col bg-white dark:bg-gray-800 rounded-lg p-4 w-full relative" data-coupon-id="${coupon.coupon_id}">
                                ...
                                <div class="flex flex-col mt-4 flex-grow">
                                    <h1 class="text-lg font-semibold mb-2 dark:text-white">${coupon.title}</h1>
                                    <p class="text-sm text-gray-500 dark:text-gray-400 mb-4">Valid till: ${coupon.validTill}</p>
...
Array.from(discountInPercIfApplicableElements).forEach(element => {
    ...
    function displayCityAreaSuggestions(predictions) {
        ...
        predictions.forEach((prediction) => {
            ...
            suggestionItem.classList.add('p-2', 'hover:bg-gray-100', 'dark:hover:bg-gray-700', 'cursor-pointer', 'flex', 'items-center', 'dark:text-white');
