<script>
    const addressModal = document.getElementById('addressModal');
    let latitude;
    let longitude;
    let map_address;

    async function fetchAddressList() {
        try {
            const response = await fetch('/fetchAddressList', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            });

            const result = await response.json();

            if (result.status === 'success') {
                const {
                    addressesWithInActiveStatus,
                    addressWithActiveStatus
                } = result;

                // Clear the current addresses in the DOM
                const addressDiv = document.querySelector('.address-div');
                if (!addressDiv) {
                    console.error('Address div not found');
                    return;
                }
                
                addressDiv.innerHTML = '';

                // Append Active Address
                if (addressWithActiveStatus) {
                    const activeAddressHTML = generateAddressHTML(addressWithActiveStatus, true);
                    addressDiv.insertAdjacentHTML('beforeend', activeAddressHTML);
                }

                // Append Inactive Addresses
                if (addressesWithInActiveStatus.length > 0) {
                    addressesWithInActiveStatus.forEach(inactiveAddress => {
                        const inactiveAddressHTML = generateAddressHTML(inactiveAddress, false);
                        addressDiv.insertAdjacentHTML('beforeend', inactiveAddressHTML);
                    });
                }

                // If no addresses found
                if (!addressWithActiveStatus && addressesWithInActiveStatus.length === 0) {
                    addressDiv.innerHTML = '<p>No addresses found.</p>';
                }
            } else {
                console.error('Failed to fetch addresses:', result.message);
                showToast('Failed to fetch addresses', 'error');
            }
        } catch (error) {
            console.error('Error fetching address list:', error);
            showToast('Error loading addresses', 'error');
        }
    }

    // Helper function to generate address card HTML
    function generateAddressHTML(address, isActive) {
        const iconClass = address.address_type === 'Home' ?
            'fi fi-rr-home' :
            address.address_type === 'Work' ?
            'fi fi-rr-building' :
            'fi fi-rr-marker';

        const bgColor = isActive ? 'bg-[#FFF4F1] border-red-400' : 'bg-[#F7F7F7] border-[#F7F7F7]';
        const deleteBtnShow = isActive ? '' : '<div class="w-1/5 flex justify-end items-end"><i class="fi fi-rr-trash text-red-500 text-lg" onclick="event.stopPropagation(); deleteAddress(' + address.id + ');"></i></div>';

        // Escape HTML to prevent XSS
        const escapeHtml = (str) => {
            const div = document.createElement('div');
            div.textContent = str || '';
            return div.innerHTML;
        };

        return `<div class="w-full flex ${bgColor} py-2 px-2 rounded-lg cursor-pointer border mb-2 md:mb-0 address-card ${isActive ? 'active' : ''}" onclick="setActiveAddress(this, ${address.id})">
                <div class="w-4/5 pr-4" onclick="event.stopPropagation(); setActiveAddress(this.parentElement, ${address.id});">
                    <span class="text-base font-medium capitalize">
                        <i class="${iconClass}"></i>
                        ${escapeHtml(address.address_type)}:
                    </span>
                    <span class="text-base font-medium capitalize mb-1">${escapeHtml(address.user_name)}</span>
                    <span class="block text-sm leading-6">${escapeHtml(address.user_mobile)}</span>
                    <span class="block text-sm leading-6">${escapeHtml(address.user_email)}</span>
                    <span class="block text-sm leading-6">${escapeHtml(address.flat)},</span>
                    <span class="block text-sm leading-6">${escapeHtml(address.address)},</span>
                    <span class="block text-sm leading-6">${escapeHtml(address.area)}, ${escapeHtml(address.city)},</span>
                    <span class="block text-sm leading-6">${escapeHtml(address.state)},</span>
                    <span class="block text-sm leading-6">${escapeHtml(address.pincode)}</span>
                </div>
                ${deleteBtnShow}
            </div>`;
    }

    function closeAddressPopup() {
        if (addressModal) {
            addressModal.classList.add('hidden');
            document.body.classList.remove('modal-open');
        }
    }

    function openAddressPopup() {
        if (!addressModal) {
            console.error("Modal element not found.");
            return;
        }

        addressModal.classList.remove('hidden');
        document.body.classList.add('modal-open');

        // Retrieve location data from localStorage
        const locationData = JSON.parse(localStorage.getItem('location'));

        if (!locationData || !locationData.lat || !locationData.lng) {
            console.error("Location data is missing or incomplete in localStorage.");
            showToast('Location data missing', 'error');
            return;
        }

        latitude = locationData.lat;
        longitude = locationData.lng;

        const currLocation = {
            lat: locationData.lat,
            lng: locationData.lng,
        };

        // Check if Google Maps is available
        if (typeof google === 'undefined' || !google.maps) {
            console.error('Google Maps not loaded');
            showToast('Map service unavailable', 'error');
            return;
        }

        // Initialize map
        const mapElement = document.getElementById("map");
        if (!mapElement) {
            console.error('Map element not found');
            return;
        }

        const map = new google.maps.Map(mapElement, {
            zoom: 15,
            center: currLocation,
        });

        // Place a draggable marker at the current location
        const marker = new google.maps.Marker({
            position: currLocation,
            map: map,
            title: "Your location",
            draggable: true,
        });

        // Fetch delivery area status
        fetchIsInDeliveryArea(locationData.lat, locationData.lng);

        // Event listener to update latitude and longitude after moving the marker
        marker.addListener('dragend', function(event) {
            const newLat = event.latLng.lat();
            const newLng = event.latLng.lng();

            latitude = newLat;
            longitude = newLng;

            fetchIsInDeliveryArea(newLat, newLng);
        });
    }

    function fetchIsInDeliveryArea(lat, lng) {
        fetch('/fetchIsInDeliveryArea', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json',
                },
                body: JSON.stringify({
                    lat: lat,
                    lng: lng,
                }),
            })
            .then(response => {
                if (!response.ok) {
                    throw new Error("Network response was not ok");
                }
                return response.json();
            })
            .then(result => {
                console.log(result);
                const areaNotFoundElements = document.getElementsByClassName('areaNotFound');
                const addressFormElements = document.getElementsByClassName('addressForm');

                if (result.status === 'success') {
                    fetchAddress(lat, lng);
                    // Hide elements with the 'areaNotFound' class
                    for (let element of areaNotFoundElements) {
                        element.classList.add('hidden');
                    }
                    // Show elements with the 'addressForm' class by removing the 'hidden' class
                    for (let element of addressFormElements) {
                        element.classList.remove('hidden');
                    }

                    console.log("Delivery is available in this area.");
                } else {
                    // Hide elements with the 'addressForm' class
                    for (let element of addressFormElements) {
                        element.classList.add('hidden');
                    }
                    // Show elements with the 'areaNotFound' class by removing the 'hidden' class
                    for (let element of areaNotFoundElements) {
                        element.classList.remove('hidden');
                    }

                    console.log("We are not available at this location.");
                }
            })
            .catch(error => {
                console.error("Error checking delivery area:", error);
                showToast('Error checking delivery area', 'error');
            });
    }

    function fetchAddress(lat, lng) {
        if (typeof google === 'undefined' || !google.maps) {
            console.error('Google Maps not loaded');
            return;
        }

        const geocoder = new google.maps.Geocoder();
        const latlng = {
            lat,
            lng
        };

        geocoder.geocode({
            location: latlng
        }, (results, status) => {
            console.log(results);
            if (status === "OK" && results && results[0]) {
                map_address = results[0].formatted_address;
                
                // Populate locationData with address components
                results[0].address_components.forEach(component => {
                    const types = component.types;
                    const areaInput = document.getElementById('area');
                    const cityInput = document.getElementById('city');
                    const stateInput = document.getElementById('state');
                    const pincodeInput = document.getElementById('pincode');

                    if (types.includes("sublocality") || types.includes("neighborhood")) {
                        if (areaInput) areaInput.value = component.long_name;
                    } else if (types.includes("locality")) {
                        if (cityInput) cityInput.value = component.long_name;
                    } else if (types.includes("administrative_area_level_1")) {
                        if (stateInput) stateInput.value = component.long_name;
                    } else if (types.includes("postal_code")) {
                        if (pincodeInput) pincodeInput.value = component.long_name;
                    }
                });
            }
        });
    }

    function selectAddressType(addressType) {
        // Update the hidden input value
        const addressTypeInput = document.getElementById('address_type');
        if (addressTypeInput) {
            addressTypeInput.value = addressType;
        }

        // Get all the location div elements
        const locationDivs = document.querySelectorAll('.flex.space-x-4 > div');

        // Iterate over the divs and apply/remove classes based on selection
        locationDivs.forEach(div => {
            if (div.textContent.trim() === addressType) {
                // Add selected styles
                div.classList.add('border-green-700', 'bg-green-100', 'shadow-md');
                div.classList.remove('border-gray-300');
            } else {
                // Reset unselected styles
                div.classList.remove('border-green-700', 'bg-green-100', 'shadow-md');
                div.classList.add('border-gray-300');
            }
        });
    }

    // Form submission handler
    const addressForm = document.querySelector('form.addressForm');
    if (addressForm) {
        addressForm.addEventListener('submit', async (event) => {
            event.preventDefault();

            const address_type = document.getElementById('address_type')?.value || '';
            const address = document.getElementById('address')?.value.trim() || '';
            const area = document.getElementById('area')?.value.trim() || '';
            const city = document.getElementById('city')?.value.trim() || '';
            const state = document.getElementById('state')?.value.trim() || '';
            const pincode = document.getElementById('pincode')?.value.trim() || '';
            const flat = document.getElementById('flat')?.value.trim() || '';
            const floor = document.getElementById('floor')?.value.trim() || '';
            const user_name = document.getElementById('user_name')?.value.trim() || '';
            const user_mobile = document.getElementById('user_mobile')?.value.trim() || '';

            const messageDiv = document.getElementById('message');

            // Basic validation
            if (!address || !area || !city || !state || !pincode || !flat || !user_mobile || !user_name) {
                if (messageDiv) {
                    messageDiv.textContent = 'Please enter required fields';
                    messageDiv.className = 'text-red-500 text-sm mt-1';
                }
                return;
            }

            // Mobile validation - Replace <?= $country['validation_no'] ?> with actual number
            const validationLength = 10; // Replace with actual validation length
            const mobileRegex = new RegExp(`^\\d{${validationLength}}$`);
            
            if (!mobileRegex.test(user_mobile)) {
                const userMobileError = document.getElementById('userMobileError');
                const userMobileInput = document.getElementById('user_mobile');
                
                if (userMobileError) {
                    userMobileError.textContent = `Please enter a valid ${validationLength}-digit mobile number.`;
                    userMobileError.classList.remove('hidden');
                }
                if (userMobileInput && userMobileInput.parentElement) {
                    userMobileInput.parentElement.classList.add('border-red-500');
                }
                return;
            }

            try {
                const response = await fetch('/saveAddress', {
                    method: 'POST',
                    headers: {
                        'Content-Type': 'application/json'
                    },
                    body: JSON.stringify({
                        address,
                        area,
                        city,
                        state,
                        pincode,
                        latitude,
                        longitude,
                        map_address,
                        address_type,
                        flat,
                        floor,
                        user_name,
                        user_mobile
                    }),
                });

                const result = await response.json();

                // Handle success or error response
                if (result.status === 'success') {
                    event.target.reset();
                    closeAddressPopup();
                    calculateOrderSummery();
                    fetchAddressList();
                    showToast(result.message || 'Address saved successfully', 'success');
                } else {
                    console.log(result.message);
                    showToast(result.message || 'Failed to save address', 'error');
                }
            } catch (error) {
                console.error('Error saving address:', error);
                if (messageDiv) {
                    messageDiv.innerHTML = `<p class="text-red-500 text-sm">Error: ${error.message}</p>`;
                }
            }
        });
    }

    async function calculateOrderSummery() {
        const wallet = JSON.parse(localStorage.getItem('wallet')) || {
            wallet_applied: 0,
            remaining_wallet_balance: 0
        };

        const appliedCoupon = JSON.parse(localStorage.getItem('appliedCoupon')) || {
            coupon_id: 0,
            coupon_code: '',
            coupon_amount: 0,
            coupon_minOrderAmount: 0
        };

        // Get seller_id from PHP
        const seller_id = <?= $seller_id ?> || 0;

        try {
            const response = await fetch('/verifyOrderDetails', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    wallet,
                    appliedCoupon,
                    seller_id: seller_id
                }),
            });

            const result = await response.json();
            console.log(result);

            // Handle success or error response
            if (result.status === 'success') {
                const subtotal = document.getElementsByClassName('subtotal');
                const taxTotal = document.getElementsByClassName('taxTotal');
                const deliveryCharge = document.getElementsByClassName('deliveryCharge');
                const grand_total = document.getElementsByClassName('grand_total');

                Array.from(subtotal).forEach(element => {
                    element.innerText = result.subTotal || 0;
                });
                Array.from(taxTotal).forEach(element => {
                    element.innerText = result.taxTotal || 0;
                });
                Array.from(deliveryCharge).forEach(element => {
                    element.innerText = result.deliveryCharge || 0;
                });

                Array.from(grand_total).forEach(element => {
                    const grandTotal =
                        (parseFloat(result.subTotal) || 0) +
                        (parseFloat(result.taxTotal) || 0) +
                        (parseFloat(result.additional_charge) || 0) +
                        (parseFloat(result.deliveryCharge) || 0) +
                        (parseFloat(typeof getDeliveryTip === 'function' ? getDeliveryTip() : 0) || 0) -
                        (parseFloat(result.coupon_amount) || 0) -
                        (parseFloat(result.wallet_applied) || 0);

                    // Round to 2 decimal places
                    element.innerText = grandTotal.toFixed(2);
                });

                const couponAmount = appliedCoupon.coupon_amount || 0;
                const walletApplied = wallet.wallet_applied || 0;
                const remainingWalletBalance = wallet.remaining_wallet_balance || 0;

                const couponAmountElements = document.getElementsByClassName('couponAmount');
                const walletAppliedElements = document.getElementsByClassName('wallet_applied');
                const remainingWalletBalanceElements = document.getElementsByClassName('remaining_wallet_balance');

                const applyCouponDiv = document.getElementById('applyCouponDiv');
                const couponAppliedDiv = document.getElementById('couponAppliedDiv');

                if (couponAmount > 0) {
                    if (applyCouponDiv) applyCouponDiv.classList.add('hidden');
                    if (couponAppliedDiv) couponAppliedDiv.classList.remove('hidden');
                } else {
                    if (applyCouponDiv) applyCouponDiv.classList.remove('hidden');
                    if (couponAppliedDiv) couponAppliedDiv.classList.add('hidden');
                }

                Array.from(couponAmountElements).forEach(element => {
                    element.innerText = couponAmount;
                });

                if (wallet && wallet.wallet_applied) {
                    Array.from(walletAppliedElements).forEach(element => {
                        element.innerText = walletApplied;
                    });

                    Array.from(remainingWalletBalanceElements).forEach(element => {
                        element.innerHTML = remainingWalletBalance >= 0 ?
                            `(${remainingWalletBalance} remaining) <i class="fi fi-rr-trash text-red-500 text-sm" onclick="event.stopPropagation(); removeWallet()"></i>` :
                            "";
                    });
                }
            } else {
                console.error('Failed to calculate order summary:', result.message);
            }
        } catch (error) {
            console.error('Error calculating order summary:', error);
        }
    }

    async function setActiveAddress(card, address_id) {
        removeCoupon();
        removeWallet();

        localStorage.setItem('wallet', JSON.stringify({
            wallet_applied: 0,
            remaining_wallet_balance: 0
        }));

        localStorage.setItem('appliedCoupon', JSON.stringify({
            coupon_id: 0,
            coupon_code: '',
            coupon_amount: 0,
            coupon_minOrderAmount: 0
        }));

        // Remove 'active' class from all address cards
        const allAddressCards = document.querySelectorAll('.address-card');
        allAddressCards.forEach((addressCard) => {
            addressCard.classList.remove('bg-[#FFF4F1]', 'border-red-400');
        });

        // Add 'active' class to the clicked card
        if (card) {
            card.classList.add('bg-[#FFF4F1]', 'border-red-400');
        }

        try {
            const response = await fetch('/activeAddress', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    address_id
                }),
            });

            const result = await response.json();
            console.log(result);

            if (result.status === 'success') {
                calculateOrderSummery();
                showToast(result.message || 'Address activated', 'success');
            } else {
                showToast(result.message || 'Failed to activate address', 'error');
            }
        } catch (error) {
            console.error('Error activating address:', error);
            showToast('Error activating address', 'error');
        }
    }

    async function deleteAddress(address_id) {
        try {
            const response = await fetch('/deleteAddress', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    address_id
                }),
            });

            const result = await response.json();

            if (result.status === 'success') {
                console.log(result);
                fetchAddressList();
                showToast(result.message || 'Address deleted', "success");
            } else {
                console.log(result);
                showToast(result.message || 'Failed to delete address', "error");
            }
        } catch (error) {
            console.error('Error deleting address:', error);
            showToast('Error deleting address', 'error');
        }
    }

    function selectDeliveryMethod(method) {
        // Save the selected delivery method in localStorage
        localStorage.setItem("deliveryMethod", method);

        // Select all delivery method cards
        const allCards = document.querySelectorAll('.deliveryMethod');

        // Loop through each card and remove the 'active' styles
        allCards.forEach(card => {
            card.classList.remove('bg-green-100', 'border-green-700', 'text-green-700');
            card.classList.add('bg-white', 'border-green-700', 'text-green-700');
        });

        // Select the clicked card and apply the active styles
        const selectedCard = document.getElementById(method);
        if (selectedCard) {
            selectedCard.classList.add('bg-green-100', 'border-green-700', 'text-green-700');
            selectedCard.classList.remove('bg-white');
        }

        const deliveryDateDiv = document.getElementById('deliveryDateDiv');
        const deliveryTimeDiv = document.getElementById('deliveryTimeDiv');

        if (method === 'scheduledDelivery') {
            if (deliveryDateDiv) {
                deliveryDateDiv.classList.remove('hidden');
                deliveryDateDiv.classList.add('block');
            }
            if (deliveryTimeDiv) {
                deliveryTimeDiv.classList.remove('hidden');
                deliveryTimeDiv.classList.add('block');
            }
        } else {
            if (deliveryDateDiv) {
                deliveryDateDiv.classList.remove('block');
                deliveryDateDiv.classList.add('hidden');
            }
            if (deliveryTimeDiv) {
                deliveryTimeDiv.classList.remove('block');
                deliveryTimeDiv.classList.add('hidden');
            }
        }
    }

    function loadDeliveryMethod() {
        // Get the stored delivery method from localStorage
        const storedMethod = localStorage.getItem("deliveryMethod");

        if (storedMethod) {
            const allCards = document.querySelectorAll('.deliveryMethod');

            allCards.forEach(card => {
                card.classList.remove('bg-green-100', 'border-green-700', 'text-green-700');
                card.classList.add('bg-white', 'border-green-700', 'text-green-700');
            });

            const selectedCard = document.getElementById(storedMethod);
            if (selectedCard) {
                selectedCard.classList.add('bg-green-100', 'border-green-700', 'text-green-700');
                selectedCard.classList.remove('bg-white');
            }

            const deliveryDateDiv = document.getElementById('deliveryDateDiv');
            const deliveryTimeDiv = document.getElementById('deliveryTimeDiv');

            if (storedMethod === 'scheduledDelivery') {
                if (deliveryDateDiv) {
                    deliveryDateDiv.classList.remove('hidden');
                    deliveryDateDiv.classList.add('block');
                }
                if (deliveryTimeDiv) {
                    deliveryTimeDiv.classList.remove('hidden');
                    deliveryTimeDiv.classList.add('block');
                }
            } else {
                if (deliveryDateDiv) {
                    deliveryDateDiv.classList.remove('block');
                    deliveryDateDiv.classList.add('hidden');
                }
                if (deliveryTimeDiv) {
                    deliveryTimeDiv.classList.remove('block');
                    deliveryTimeDiv.classList.add('hidden');
                }
            }
        }
    }

    async function setActiveDate(button) {
        // Remove 'active' class from all buttons
        const allButtons = document.querySelectorAll('.swiper-slide button.date');
        allButtons.forEach(btn => btn.classList.remove('bg-green-100', 'border-green-500', 'text-green-700'));

        // Add 'active' class to the clicked button
        if (button) {
            button.classList.add('bg-green-100', 'border-green-500', 'text-green-700');
        }

        const selectedDate = button.getAttribute("data-date");
        const currentYear = new Date().getFullYear();
        const formattedDate = formatDate(selectedDate, currentYear);
        localStorage.setItem("activeDate", formattedDate);

        // Clear the timeslot div and active time in localStorage
        const timeslotDiv = document.getElementById('timeslotDiv');
        if (timeslotDiv) {
            timeslotDiv.innerHTML = '';
        }
        localStorage.removeItem('activeTime');

        try {
            const response = await fetch('/getTimeSlot', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    date: formattedDate
                })
            });

            const result = await response.json();

            if (result.status === 'success') {
                appendTimeSlots(result.data || []);
            } else {
                console.error(result.message);
                appendTimeSlots([]);
            }
        } catch (error) {
            console.error('Error fetching time slots:', error);
            appendTimeSlots([]);
        }
    }

    function checkActiveDateOnLoad() {
        const activeDate = localStorage.getItem("activeDate");
        if (activeDate) {
            const allButtons = document.querySelectorAll('.swiper-slide button.date');
            allButtons.forEach(button => {
                const buttonDate = button.getAttribute("data-date");
                if (activeDate === formatDate(buttonDate, new Date().getFullYear())) {
                    button.classList.add('bg-green-100', 'border-green-500', 'text-green-700');
                }
            });
        }
    }

    function formatDate(shortDate, year) {
        const months = {
            Jan: "01", Feb: "02", Mar: "03", Apr: "04",
            May: "05", Jun: "06", Jul: "07", Aug: "08",
            Sep: "09", Oct: "10", Nov: "11", Dec: "12"
        };

        const [month, day] = shortDate.split(" ");
        const monthNumber = months[month];
        const dayNumber = day.padStart(2, "0");
        return `${year}-${monthNumber}-${dayNumber}`;
    }

    function appendTimeSlots(timeslots) {
        const parentDiv = document.getElementById('timeslotDiv');
        if (!parentDiv) {
            console.error('Timeslot div not found');
            return;
        }

        parentDiv.innerHTML = '';

        if (!timeslots || timeslots.length === 0) {
            parentDiv.innerHTML = '<p class="text-gray-500">No available time slots for the selected date.</p>';
            return;
        }

        timeslots.forEach(slot => {
            const mintime = formatTime(slot.mintime);
            const maxtime = formatTime(slot.maxtime);

            const slotElement = document.createElement('div');
            slotElement.className = 'swiper-slide';

            slotElement.innerHTML = `
                <button
                    class="flex flex-col text-sm whitespace-nowrap border py-2 px-6 rounded-lg bg-gray-100 border-gray-500 text-gray-700 time"
                    onclick="setActiveTime(this)" data-time="${mintime} - ${maxtime}" >
                    ${mintime} to ${maxtime}
                </button>
            `;

            parentDiv.appendChild(slotElement);
        });
    }

    function formatTime(time) {
        const [hour, minute] = time.split('.');
        const hourInt = parseInt(hour);
        const minuteInt = parseInt((minute || '0').padEnd(2, '0'));

        const ampm = hourInt >= 12 ? 'PM' : 'AM';
        const formattedHour = hourInt % 12 || 12;

        return `${formattedHour}.${minuteInt.toString().padStart(2, '0')} ${ampm}`;
    }

    function setActiveTime(button) {
        const allButtons = document.querySelectorAll('.swiper-slide button.time');
        allButtons.forEach(btn => btn.classList.remove('bg-green-100', 'border-green-500', 'text-green-700'));

        if (button) {
            button.classList.add('bg-green-100', 'border-green-500', 'text-green-700');
        }

        const selectedTime = button.getAttribute("data-time");
        localStorage.setItem("activeTime", selectedTime);
    }

    const couponModal = document.getElementById('couponModal');

    async function openCouponPopup() {
        const couponListDiv = document.getElementById('couponListDiv');

        if (!couponModal || !couponListDiv) {
            console.error("Modal element or coupon list container not found.");
            return;
        }

        couponModal.classList.remove('hidden');
        document.body.classList.add('modal-open');

        try {
            const response = await fetch('/getCouponList', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                }
            });

            const result = await response.json();

            if (result.status === 'success') {
                couponListDiv.innerHTML = '';

                (result.data || []).forEach(coupon => {
                    const couponCard = document.createElement('div');
                    couponCard.innerHTML = `
                        <div class="flex flex-col bg-white rounded-lg p-4 w-full relative" data-coupon-id="${coupon.id}">
                            <div class="flex items-center">
                                <img src="${coupon.coupon_img || ''}" alt="Coupon" class="w-8 h-8 rounded-lg object-cover mr-4" />
                                <h3 class="py-1 px-2 rounded font-medium text-xs bg-[#FFDB1F] text-black">Code: ${coupon.code}</h3>
                            </div>
                            <div class="flex flex-col mt-4 flex-grow">
                                <h1 class="text-lg font-semibold mb-2">${coupon.title}</h1>
                                <p class="text-sm text-gray-500 mb-4">Valid till: ${coupon.validTill}</p>
                                <button id="coupon_btn_id_${coupon.id}" onclick="applyCoupon(${coupon.id}, '${coupon.code}', ${coupon.amount}, ${coupon.minOrderAmount})"
                                        class="mt-auto self-end bg-green-700 text-white py-2 px-4 rounded-lg text-sm hover:bg-green-900 transition-all">
                                    Apply
                                </button>
                            </div>
                        </div>
                    `;

                    const appliedCoupon = JSON.parse(localStorage.getItem('appliedCoupon'));
                    if (appliedCoupon && +appliedCoupon.coupon_id === +coupon.id) {
                        const couponCardElement = couponCard.querySelector(`[data-coupon-id="${coupon.id}"]`);
                        if (couponCardElement) {
                            couponCardElement.classList.add('bg-green-100', 'border', 'border-green-500');
                            const couponBtn = couponCard.querySelector(`#coupon_btn_id_${coupon.id}`);
                            if (couponBtn) {
                                couponBtn.innerText = 'Applied';
                                couponBtn.disabled = true;
                            }

                            const applyCouponDiv = document.getElementById('applyCouponDiv');
                            const couponAppliedDiv = document.getElementById('couponAppliedDiv');
                            if (applyCouponDiv) applyCouponDiv.classList.add('hidden');
                            if (couponAppliedDiv) couponAppliedDiv.classList.remove('hidden');

                            const couponAmountElements = document.getElementsByClassName('couponAmount');
                            Array.from(couponAmountElements).forEach(element => {
                                element.innerText = coupon.amount;
                            });
                        }
                    }

                    couponListDiv.appendChild(couponCard);
                });
            } else {
                console.error(result.message);
                showToast(result.message || 'Failed to load coupons', 'error');
            }
        } catch (error) {
            console.error('Error fetching coupons:', error);
            showToast('Error loading coupons', 'error');
        }
    }

    function closeCouponPopup() {
        if (couponModal) {
            couponModal.classList.add('hidden');
            document.body.classList.remove('modal-open');
        }
    }

    async function applyCoupon(coupon_id, coupon_code, coupon_amount, coupon_minOrderAmount) {
        const wallet = JSON.parse(localStorage.getItem('wallet')) || {
            wallet_applied: 0,
            remaining_wallet_balance: 0
        };

        try {
            const response = await fetch('/applyCoupon', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    coupon_id,
                    coupon_code,
                    coupon_amount,
                    coupon_minOrderAmount,
                    wallet
                }),
            });

            const result = await response.json();
            console.log(result);

            if (result.status === 'success') {
                const couponData = {
                    coupon_id,
                    coupon_code,
                    coupon_amount,
                    coupon_minOrderAmount
                };
                localStorage.setItem('appliedCoupon', JSON.stringify(couponData));

                const couponCard = document.querySelector(`[data-coupon-id="${coupon_id}"]`);

                if (couponCard) {
                    couponCard.classList.add('bg-green-100', 'border', 'border-green-500');

                    const couponBtn = couponCard.querySelector(`#coupon_btn_id_${coupon_id}`);
                    if (couponBtn) {
                        couponBtn.innerText = 'Applied';
                        couponBtn.disabled = true;
                    }

                    const applyCouponDiv = document.getElementById('applyCouponDiv');
                    const couponAppliedDiv = document.getElementById('couponAppliedDiv');
                    if (applyCouponDiv) applyCouponDiv.classList.add('hidden');
                    if (couponAppliedDiv) couponAppliedDiv.classList.remove('hidden');

                    const couponAmountElements = document.getElementsByClassName('couponAmount');
                    Array.from(couponAmountElements).forEach(element => {
                        element.innerText = coupon_amount;
                    });

                    calculateOrderSummery();
                }
                showToast(result.message || 'Coupon applied', "success");
                closeCouponPopup();
            } else {
                showToast(result.message || 'Failed to apply coupon', "error");
                console.error(result.message);
                closeCouponPopup();
            }
        } catch (error) {
            console.error('Error applying coupon:', error);
            showToast('Error applying coupon', 'error');
        }
    }

    async function removeCoupon() {
        try {
            const response = await fetch('/removeCoupon', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
            });

            const result = await response.json();
            console.log(result);

            if (result.status === 'success') {
                const applyCouponDiv = document.getElementById('applyCouponDiv');
                const couponAppliedDiv = document.getElementById('couponAppliedDiv');
                if (applyCouponDiv) applyCouponDiv.classList.remove('hidden');
                if (couponAppliedDiv) couponAppliedDiv.classList.add('hidden');

                localStorage.removeItem("appliedCoupon");
                const couponAmount = document.getElementsByClassName('couponAmount');

                Array.from(couponAmount).forEach(element => {
                    element.innerText = 0;
                });
                calculateOrderSummery();

                showToast(result.message || 'Coupon removed', "success");
            } else {
                showToast(result.message || 'Failed to remove coupon', "error");
            }
        } catch (error) {
            console.error('Error removing coupon:', error);
            showToast('Error removing coupon', 'error');
        }
    }

    window.addEventListener('DOMContentLoaded', () => {
        checkActiveDateOnLoad();
        fetchAddressList();
        loadDeliveryMethod();
        calculateOrderSummery();
    });

    async function applyWallet() {
        const appliedCoupon = JSON.parse(localStorage.getItem('appliedCoupon'));

        try {
            const response = await fetch('/applyWallet', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    appliedCoupon
                }),
            });

            const result = await response.json();
            console.log(result);

            if (result.status === 'success') {
                const wallet_applied = document.getElementsByClassName('wallet_applied');
                const remaining_wallet_balance = document.getElementsByClassName('remaining_wallet_balance');

                Array.from(wallet_applied).forEach(element => {
                    element.innerText = result.data.wallet_applied || 0;
                });

                Array.from(remaining_wallet_balance).forEach(element => {
                    element.innerHTML = `(${result.data.remaining_wallet_balance || 0} remaining) <i class="fi fi-rr-trash text-red-500 text-sm" onclick="event.stopPropagation(); removeWallet()"></i>`;
                });

                localStorage.setItem('wallet', JSON.stringify({
                    wallet_applied: result.data.wallet_applied || 0,
                    remaining_wallet_balance: result.data.remaining_wallet_balance || 0
                }));

                calculateOrderSummery();
                showToast(result.message || 'Wallet applied', "success");
            } else {
                console.error(result.message);
                showToast(result.message || 'Failed to apply wallet', "error");
            }
        } catch (error) {
            console.error('Error applying wallet:', error);
            showToast('Error applying wallet', 'error');
        }
    }

    async function removeWallet() {
        const appliedCoupon = JSON.parse(localStorage.getItem('appliedCoupon'));
        let coupon_id = 0;
        if (appliedCoupon && appliedCoupon.coupon_amount) {
            coupon_id = appliedCoupon.coupon_id;
        }

        try {
            const response = await fetch('/removeWallet', {
                method: 'POST',
                headers: {
                    'Content-Type': 'application/json'
                },
                body: JSON.stringify({
                    coupon_id
                }),
            });

            const result = await response.json();
            console.log(result);

            if (result.status === 'success') {
                localStorage.removeItem('wallet');

                const remaining_wallet_balance = document.getElementsByClassName('remaining_wallet_balance');
                const wallet_applied = document.getElementsByClassName('wallet_applied');

                Array.from(remaining_wallet_balance).forEach(element => {
                    element.innerHTML = `(${result.data.remaining_wallet_balance || 0} apply)`;
                });
                Array.from(wallet_applied).forEach(element => {
                    element.innerHTML = 0;
                });

                calculateOrderSummery();
                showToast(result.message || 'Wallet removed', "success");
            } else {
                console.error(result.message);
                showToast(result.message || 'Failed to remove wallet', "error");
            }
        } catch (error) {
            console.error('Error removing wallet:', error);
            showToast('Error removing wallet', 'error');
        }
    }

    localStorage.setItem('paymentMethode', 0);

    function setPaymentMethode(id) {
        localStorage.setItem('paymentMethode', id);
        const paymentElements = document.querySelectorAll('[id*="paymentMethod_"]');

        paymentElements.forEach(element => {
            element.classList.remove('border-red-400', 'bg-[#FFF4F1]');
        });

        const selectedElement = document.getElementById('paymentMethod_' + id);
        if (selectedElement) {
            selectedElement.classList.add('border-red-400', 'bg-[#FFF4F1]');
        }

        const paypalContainer = document.getElementById('paypal-button-container');
        const verifyOrderBtn = document.getElementById('verifyOrderDetails');

        if (id == 3) {
            if (paypalContainer) paypalContainer.classList.remove('hidden');
            if (verifyOrderBtn) verifyOrderBtn.classList.add('hidden');
        } else {
            if (paypalContainer) paypalContainer.classList.add('hidden');
            if (verifyOrderBtn) verifyOrderBtn.classList.remove('hidden');
        }
    }

    async function verifyOrderDetails() {
    let paymentMethode = localStorage.getItem('paymentMethode');
    let deliveryMethod = localStorage.getItem('deliveryMethod');

    const appliedCoupon = JSON.parse(localStorage.getItem('appliedCoupon'));
    const wallet = JSON.parse(localStorage.getItem('wallet'));
    const activeDate = localStorage.getItem('activeDate');
    const activeTime = localStorage.getItem('activeTime');

    if (activeDate == null && deliveryMethod == 'scheduledDelivery') {
        showToast('Select Date', "error");
        return false;
    }
    if (activeTime == null && deliveryMethod == 'scheduledDelivery') {
        showToast('Select Time', "error");
        return false;
    }

    // Get seller_id - replace with actual value from PHP
    const seller_id = <?= $seller_id ?> || 0;

    try {
        const response = await fetch('/verifyOrderDetails', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                appliedCoupon,
                wallet,
                activeDate,
                activeTime,
                paymentMethode,
                deliveryMethod,
                seller_id: seller_id,
                deliveryInstruction: getDeliveryInstruction?.() || '',
                deliveryTip: getDeliveryTip?.() || 0,
                gstNumber: getGstNumber?.() || ''
            }),
        });

        const result = await response.json();

        if (result.status === 'success') {
            const subtotal = document.getElementsByClassName('subtotal');
            const taxTotal = document.getElementsByClassName('taxTotal');
            const deliveryCharge = document.getElementsByClassName('deliveryCharge');
            const grand_total = document.getElementsByClassName('grand_total');

            Array.from(subtotal).forEach(element => {
                element.innerText = result.subTotal || 0;
            });
            Array.from(taxTotal).forEach(element => {
                element.innerText = result.taxTotal || 0;
            });
            Array.from(deliveryCharge).forEach(element => {
                element.innerText = result.deliveryCharge || 0;
            });

            if (wallet !== null) {
                const remaining_wallet_balance = document.getElementsByClassName('remaining_wallet_balance');
                const wallet_applied = document.getElementsByClassName('wallet_applied');

                Array.from(remaining_wallet_balance).forEach(element => {
                    element.innerHTML = `(${result.remaining_wallet_balance || 0} remaining) <i class="fi fi-rr-trash text-red-500 text-sm" onclick="event.stopPropagation(); removeWallet()"></i>`;
                });
                Array.from(wallet_applied).forEach(element => {
                    element.innerHTML = result.wallet_applied || 0;
                });

                localStorage.setItem('wallet', JSON.stringify({
                    wallet_applied: result.wallet_applied || 0,
                    remaining_wallet_balance: result.remaining_wallet_balance || 0
                }));

                const grandTotal =
                    (parseFloat(result.subTotal) || 0) +
                    (parseFloat(result.taxTotal) || 0) +
                    (parseFloat(result.additional_charge) || 0) +
                    (parseFloat(result.deliveryCharge) || 0) +
                    (parseFloat(getDeliveryTip?.() || 0)) -
                    (parseFloat(result.coupon_amount) || 0) -
                    (parseFloat(result.wallet_applied) || 0);

                Array.from(grand_total).forEach(element => {
                    element.innerText = grandTotal.toFixed(2);
                });

                if (grandTotal == 0) {
                    await placeCODOrder(appliedCoupon, wallet, activeDate, activeTime, deliveryMethod, seller_id);
                    return true;
                }
            } else {
                Array.from(grand_total).forEach(element => {
                    const grandTotal =
                        (parseFloat(result.subTotal) || 0) +
                        (parseFloat(result.taxTotal) || 0) +
                        (parseFloat(result.additional_charge) || 0) +
                        (parseFloat(result.deliveryCharge) || 0) +
                        (parseFloat(getDeliveryTip?.() || 0)) -
                        (parseFloat(result.coupon_amount) || 0);

                    element.innerText = grandTotal.toFixed(2);
                });

                localStorage.removeItem('wallet');
            }

            if (paymentMethode == 0) {
                showToast('Select Payment Method', "error");
                return false;
            }

            // Payment Method 1: Cash on Delivery
            if (paymentMethode == 1) {
                await placeCODOrder(appliedCoupon, wallet, activeDate, activeTime, deliveryMethod, seller_id);
            } 
            // Payment Method 2: Razorpay
            else if (paymentMethode == 2) {
                <?php foreach ($paymentMethods as $paymentMethod): ?>
                    <?php if ($paymentMethod['id'] == 2): ?>
                        try {
                            const razorpayResponse = await fetch('/createRazorpayOrder', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json'
                                },
                                body: JSON.stringify({
                                    appliedCoupon,
                                    wallet,
                                    activeDate,
                                    activeTime,
                                    paymentMethode,
                                    deliveryMethod,
                                    seller_id: seller_id,
                                    deliveryInstruction: getDeliveryInstruction?.() || '',
                                    deliveryTip: getDeliveryTip?.() || 0,
                                    gstNumber: getGstNumber?.() || ''
                                }),
                            });
                            const razorpayResult = await razorpayResponse.json();
                            console.log(razorpayResult);

                            if (razorpayResult.status === 'success') {
                                let razorpay_order_id = razorpayResult.razorpay_order_id;
                                let amount = razorpayResult.amount;
                                let order_id = razorpayResult.order_id;

                                var options = {
                                    "key": "<?= $paymentMethod['api_key'] ?>",
                                    "amount": amount,
                                    "currency": '<?= $country['currency_shortcut'] ?>',
                                    "name": "<?= $settings['business_name'] ?>",
                                    "description": "Purchased from <?= $settings['business_name'] ?>",
                                    "image": "<?= base_url($settings['logo']) ?>",
                                    "prefill": {
                                        "name": '<?= $user_name ?>',
                                        "email": '<?= $user_email ?>',
                                        "contact": '<?= $user_mobile ?>'
                                    },
                                    "theme": {
                                        "color": "#2B4CAB"
                                    },
                                    "order_id": razorpay_order_id,
                                    "handler": async function(res) {
                                        const verifyResponse = await fetch('/verifyRazorpayPayment', {
                                            method: 'POST',
                                            headers: {
                                                'Content-Type': 'application/json'
                                            },
                                            body: JSON.stringify({
                                                razorpay_payment_id: res.razorpay_payment_id,
                                                razorpay_order_id: res.razorpay_order_id,
                                                razorpay_signature: res.razorpay_signature,
                                                order_id: order_id,
                                                seller_id: seller_id
                                            }),
                                        });
                                        const verifyResult = await verifyResponse.json();

                                        if (verifyResult.status === 'success') {
                                            localStorage.removeItem('wallet');
                                            localStorage.removeItem('activeDate');
                                            localStorage.removeItem('activeTime');
                                            localStorage.removeItem('appliedCoupon');
                                            localStorage.setItem('paymentMethode', 0);

                                            const popupContent = `
                                                <div class="bg-white w-11/12 sm:w-96 rounded-lg p-6 shadow-lg">
                                                    <div class="flex items-center justify-center text-green-500">
                                                        <img src="${verifyResult.base_url}/assets/dist/img/success-animation.gif" alt="Success Animation"/>
                                                    </div>
                                                    <div class="text-center mt-4">
                                                        <h2 class="text-lg font-bold text-gray-800"><?php echo lang('website.order_placed_successfully'); ?>!</h2>
                                                        <p class="text-sm text-gray-600 mt-2">
                                                            <?php echo lang('website.thank_you_for_your_order_we_will_notify_you_when_its_ready_for_delivery'); ?>
                                                        </p>
                                                    </div>
                                                    <div class="flex items-center justify-center mt-6">
                                                        <a href="${verifyResult.base_url}/order-details/${order_id}" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">
                                                            <?php echo lang('website.see_your_order_details'); ?> 
                                                        </a>
                                                    </div>
                                                </div>
                                            `;

                                            const popupContainer = document.getElementById('orderResponsePopup');
                                            if (popupContainer) {
                                                popupContainer.innerHTML = popupContent;
                                                popupContainer.classList.remove('hidden');
                                            }
                                            showToast(verifyResult.message, "success");
                                        } else {
                                            showToast(verifyResult.message, "error");
                                        }
                                    }
                                };

                                var rzp1 = new Razorpay(options);
                                rzp1.open();

                            } else {
                                showToast(razorpayResult.message, "error");
                            }
                        } catch (e) {
                            console.error('Razorpay error:', e);
                            showToast('Payment initialization failed', 'error');
                        }
                    <?php endif; ?>
                <?php endforeach; ?>
            } 
            // Payment Method 4: Paystack
            else if (paymentMethode == 4) {
                <?php foreach ($paymentMethods as $paymentMethod): ?>
                    <?php if ($paymentMethod['id'] == 4): ?>
                        try {
                            const paystackResponse = await fetch('/createPaystackOrder', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json'
                                },
                                body: JSON.stringify({
                                    appliedCoupon,
                                    wallet,
                                    activeDate,
                                    activeTime,
                                    paymentMethode,
                                    deliveryMethod,
                                    seller_id: seller_id,
                                    deliveryInstruction: getDeliveryInstruction?.() || '',
                                    deliveryTip: getDeliveryTip?.() || 0,
                                    gstNumber: getGstNumber?.() || ''
                                }),
                            });
                            const paystackResult = await paystackResponse.json();
                            console.log(paystackResult);

                            if (paystackResult.status === 'success') {
                                let amount = paystackResult.amount;
                                let order_id = paystackResult.order_id;
                                const paystack = new PaystackPop();
                                paystack.newTransaction({
                                    key: '<?= $paymentMethod['api_key'] ?>',
                                    email: '<?= $user_email ?>',
                                    amount: amount * 100,
                                    currency: '<?= $country['currency_shortcut'] ?>',
                                    onSuccess: function(transaction) {
                                        fetch('<?= base_url("verifyPaystackOrder") ?>', {
                                            method: 'POST',
                                            headers: {
                                                'Content-Type': 'application/json',
                                            },
                                            body: JSON.stringify({
                                                reference: transaction.reference,
                                                transaction: transaction.transaction,
                                                amount,
                                                order_id,
                                                seller_id: seller_id
                                            })
                                        })
                                        .then(response => response.json())
                                        .then(data => {
                                            if (data.status === 'success') {
                                                localStorage.removeItem('wallet');
                                                localStorage.removeItem('activeDate');
                                                localStorage.removeItem('activeTime');
                                                localStorage.removeItem('appliedCoupon');
                                                localStorage.setItem('paymentMethode', 0);

                                                const popupContent = `
                                                    <div class="bg-white w-11/12 sm:w-96 rounded-lg p-6 shadow-lg">
                                                        <div class="flex items-center justify-center text-green-500">
                                                            <img src="${data.base_url}/assets/dist/img/success-animation.gif" alt="Success Animation"/>
                                                        </div>
                                                        <div class="text-center mt-4">
                                                            <h2 class="text-lg font-bold text-gray-800"><?php echo lang('website.order_placed_successfully'); ?>!</h2>
                                                            <p class="text-sm text-gray-600 mt-2">
                                                                <?php echo lang('website.thank_you_for_your_order_we_will_notify_you_when_its_ready_for_delivery'); ?>
                                                            </p>
                                                        </div>
                                                        <div class="flex items-center justify-center mt-6">
                                                            <a href="${data.base_url}/order-details/${order_id}" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">
                                                                <?php echo lang('website.see_your_order_details'); ?> 
                                                            </a>
                                                        </div>
                                                    </div>
                                                `;

                                                const popupContainer = document.getElementById('orderResponsePopup');
                                                if (popupContainer) {
                                                    popupContainer.innerHTML = popupContent;
                                                    popupContainer.classList.remove('hidden');
                                                }
                                                showToast(data.message, "success");
                                            } else {
                                                showToast(data.message, "error");
                                            }
                                        })
                                        .catch(error => {
                                            console.error('Error verifying payment:', error);
                                            showToast('Payment verification failed', 'error');
                                        });
                                    },
                                    onCancel: function() {
                                        showToast('Transaction cancelled.', "error");
                                    }
                                });
                            } else {
                                showToast(paystackResult.message, "error");
                            }
                        } catch (e) {
                            console.error('Paystack error:', e);
                            showToast('Payment initialization failed', 'error');
                        }
                    <?php endif; ?>
                <?php endforeach; ?>
            }
            // Payment Method 5: CashFree
            else if (paymentMethode == 5) {
                <?php foreach ($paymentMethods as $paymentMethod): ?>
                    <?php if ($paymentMethod['id'] == 5): ?>
                        try {
                            const cashfreeResponse = await fetch('/createCashFreeOrder', {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json'
                                },
                                body: JSON.stringify({
                                    appliedCoupon,
                                    wallet,
                                    activeDate,
                                    activeTime,
                                    paymentMethode,
                                    deliveryMethod,
                                    seller_id: seller_id,
                                    deliveryInstruction: getDeliveryInstruction?.() || '',
                                    deliveryTip: getDeliveryTip?.() || 0,
                                    gstNumber: getGstNumber?.() || ''
                                }),
                            });

                            const cashfreeResult = await cashfreeResponse.json();
                            console.log(cashfreeResult);

                            if (cashfreeResult.status === 'success') {
                                const cashfree_order_id = cashfreeResult.cashfree_order_id;
                                const order_id = cashfreeResult.order_id;
                                const payment_session_id = cashfreeResult.payment_session_id;

                                const cashfree = new Cashfree({
                                    mode: "sandbox", // Use "sandbox" for testing or "production" for live
                                });

                                const checkoutOptions = {
                                    paymentSessionId: payment_session_id,
                                    redirectTarget: "_modal",
                                };

                                try {
                                    const cashFreeResult = await cashfree.checkout(checkoutOptions);

                                    if (cashFreeResult.error) {
                                        console.error("Payment error or user closed the modal:", cashFreeResult.error);
                                        showToast('Payment cancelled or failed', 'error');
                                        return;
                                    }

                                    if (cashFreeResult.paymentDetails) {
                                        console.log("Payment completed, verifying payment...");
                                        console.log(cashFreeResult.paymentDetails.paymentMessage);

                                        const confirmResponse = await fetch('/confirmCashFreeOrder', {
                                            method: 'POST',
                                            headers: {
                                                'Content-Type': 'application/json'
                                            },
                                            body: JSON.stringify({
                                                order_id,
                                                payment_session_id,
                                                cashfree_order_id,
                                                seller_id: seller_id
                                            }),
                                        });

                                        const confirmResult = await confirmResponse.json();

                                        if (confirmResult.status === 'success') {
                                            localStorage.removeItem('wallet');
                                            localStorage.removeItem('activeDate');
                                            localStorage.removeItem('activeTime');
                                            localStorage.removeItem('appliedCoupon');
                                            localStorage.setItem('paymentMethode', 0);

                                            const popupContent = `
                                                <div class="bg-white w-11/12 sm:w-96 rounded-lg p-6 shadow-lg">
                                                    <div class="flex items-center justify-center text-green-500">
                                                        <img src="${confirmResult.base_url}/assets/dist/img/success-animation.gif" alt="Success Animation"/>
                                                    </div>
                                                    <div class="text-center mt-4">
                                                        <h2 class="text-lg font-bold text-gray-800"><?php echo lang('website.order_placed_successfully'); ?>!</h2>
                                                        <p class="text-sm text-gray-600 mt-2">
                                                            <?php echo lang('website.thank_you_for_your_order_we_will_notify_you_when_its_ready_for_delivery'); ?>
                                                        </p>
                                                    </div>
                                                    <div class="flex items-center justify-center mt-6">
                                                        <a href="${confirmResult.base_url}/order-details/${order_id}" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">
                                                            <?php echo lang('website.see_your_order_details'); ?> 
                                                        </a>
                                                    </div>
                                                </div>
                                            `;

                                            const popupContainer = document.getElementById('orderResponsePopup');
                                            if (popupContainer) {
                                                popupContainer.innerHTML = popupContent;
                                                popupContainer.classList.remove('hidden');
                                            }

                                            showToast(confirmResult.message, "success");
                                        } else {
                                            showToast(confirmResult.message, "error");
                                        }
                                    }
                                } catch (error) {
                                    console.error("Error during Cashfree checkout:", error);
                                    showToast('Checkout error occurred', 'error');
                                }
                            } else {
                                showToast(cashfreeResult.message, "error");
                            }
                        } catch (error) {
                            console.error("Error during payment process:", error);
                            showToast('Payment initialization failed', 'error');
                        }
                    <?php endif; ?>
                <?php endforeach; ?>
            } 
            // Payment Method 6: Stripe
            else if (paymentMethode == 6) {
                <?php foreach ($paymentMethods as $paymentMethod): ?>
                    <?php if ($paymentMethod['id'] == 6): ?>
                        try {
                            const stripe = Stripe('<?= $paymentMethod['api_key'] ?>');
                            console.log(stripe);
                            
                            // TODO: Implement Stripe payment flow
                            // This is a placeholder - you need to complete the Stripe integration
                            const elements = stripe.elements({
                                mode: 'payment',
                                currency: 'usd',
                                amount: 1099,
                            });
                            
                            showToast('Stripe integration pending', 'info');
                        } catch (error) {
                            console.error('Stripe error:', error);
                            showToast('Payment initialization failed', 'error');
                        }
                    <?php endif; ?>
                <?php endforeach; ?>
            }

        } else {
            showToast(result.message || 'Verification failed', "error");
        }
    } catch (error) {
        console.error('Error verifying order:', error);
        showToast('Error verifying order', 'error');
    }
}

// Helper function to place COD order
async function placeCODOrder(appliedCoupon, wallet, activeDate, activeTime, deliveryMethod, seller_id) {
    try {
        const response = await fetch('/placeCODOrder', {
            method: 'POST',
            headers: {
                'Content-Type': 'application/json'
            },
            body: JSON.stringify({
                appliedCoupon,
                wallet,
                activeDate,
                activeTime,
                deliveryMethod,
                seller_id: seller_id,
                deliveryInstruction: getDeliveryInstruction?.() || '',
                deliveryTip: getDeliveryTip?.() || 0,
                gstNumber: getGstNumber?.() || ''
            }),
        });

        const result = await response.json();

        if (result.status === 'success') {
            localStorage.removeItem('wallet');
            localStorage.removeItem('activeDate');
            localStorage.removeItem('activeTime');
            localStorage.removeItem('appliedCoupon');
            localStorage.setItem('paymentMethode', 0);

            const popupContent = `
                <div class="bg-white dark:bg-gray-800 w-11/12 sm:w-96 rounded-lg p-6 shadow-lg">
                    <div class="flex items-center justify-center text-green-500">
                        <img src="${result.base_url}/assets/dist/img/success-animation.gif" alt="Success Animation"/>
                    </div>
                    <div class="text-center mt-4">
                        <h2 class="text-lg font-bold text-gray-800 dark:text-white"><?php echo lang('website.order_placed_successfully'); ?>!</h2>
                        <p class="text-sm text-gray-600 dark:text-gray-400 mt-2">
                            <?php echo lang('website.thank_you_for_your_order_we_will_notify_you_when_its_ready_for_delivery'); ?>
                        </p>
                    </div>
                    <div class="flex items-center justify-center mt-6">
                        <a href="${result.base_url}/order-details/${result.order_id}" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">
                            <?php echo lang('website.see_your_order_details'); ?>
                        </a>
                    </div>
                </div>
            `;

            const popupContainer = document.getElementById('orderResponsePopup');
            if (popupContainer) {
                popupContainer.innerHTML = popupContent;
                popupContainer.classList.remove('hidden');
            }

            showToast(result.message || 'Order placed successfully', "success");
            return true;
        } else {
            showToast(result.message || 'Failed to place order', "error");
            return false;
        }
    } catch (error) {
        console.error('Error placing COD order:', error);
        showToast('Error placing order', 'error');
        return false;
    }
}

</script>
<?php foreach ($paymentMethods as $paymentMethod):
    if ($paymentMethod['id'] == 3):
?>
        <script src="https://www.paypal.com/sdk/js?client-id=<?= $paymentMethod['api_key'] ?>&currency=USD"></script>
        <script>
            paypal.Buttons({

                createOrder: function(data, actions) {

                    const appliedCoupon = JSON.parse(localStorage.getItem('appliedCoupon'))
                    const wallet = JSON.parse(localStorage.getItem('wallet'))
                    const activeDate = localStorage.getItem('activeDate')
                    const activeTime = localStorage.getItem('activeTime')
                    const deliveryMethod = localStorage.getItem('deliveryMethod')
                    let paymentMethode = localStorage.getItem('paymentMethode');

                    if (activeDate == null && deliveryMethod == 'scheduledDelivery') {
                        showToast('Select Date', "error");
                        return false
                    }
                    if (activeTime == null && deliveryMethod == 'scheduledDelivery') {
                        showToast('Select Time', "error");
                        return false
                    }
                    return fetch('/createPaypalOrder', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                            body: JSON.stringify({
                                appliedCoupon,
                                wallet,
                                activeDate,
                                activeTime,
                                deliveryMethod,
                                paymentMethode,
                                seller_id: <?= $seller_id ?>,
                                deliveryInstruction: getDeliveryInstruction?.() || '',
                                deliveryTip: getDeliveryTip?.() || 0,
                                gstNumber: getGstNumber?.() || ''
                            }),
                        })
                        .then((res) => {
                            console.log(res)
                            if (!res.ok) {
                                throw new Error('Failed to create order');
                            }
                            return res.json();
                        })
                        .then((orderData) => {
                            if (!orderData.order_id) {
                                throw new Error('Order ID is missing in the response');
                            }
                            console.log('Order Data:', orderData);
                            return orderData.order_id;
                        })
                        .catch((err) => {
                            showToast('Error in createOrder', "error");

                            console.error('Error in createOrder:', err);
                            throw err;
                        });
                },
                onApprove: function(data, actions) {
                    // Capture the payment on your backend
                    return fetch('/capturePaypalOrder', {
                            method: 'POST',
                            headers: {
                                'Content-Type': 'application/json',
                            },
                            body: JSON.stringify({
                                orderID: data.orderID,
                                seller_id: <?= $seller_id ?>

                            }),
                        })
                        .then((res) => res.json())
                        .then((captureData) => {

                            localStorage.removeItem('wallet')
                            localStorage.removeItem('activeDate')
                            localStorage.removeItem('activeTime')
                            localStorage.removeItem('appliedCoupon')
                            localStorage.setItem('paymentMethode', 0)

                            const popupContent = `
                                                        <div class="bg-white w-11/12 sm:w-96 rounded-lg p-6 shadow-lg">
                                                            <div class="flex items-center justify-center text-green-500">
                                                                <img src="${captureData.base_url}/assets/dist/img/success-animation.gif" alt="Success Animation"/>
                                                            </div>
                                                            <div class="text-center mt-4">
                                                                <h2 class="text-lg font-bold text-gray-800"><?php echo lang('website.order_placed_successfully'); ?>!</h2>
                                                                <p class="text-sm text-gray-600 mt-2">
                                                                    <?php echo lang('website.thank_you_for_your_order_we_will_notify_you_when_its_ready_for_delivery'); ?>
                                                                </p>
                                                            </div>
                                                            <div class="flex items-center justify-center mt-6">
                                                                <a href="${captureData.base_url}/order-details/${captureData.order_id}" class="bg-green-500 text-white px-4 py-2 rounded-lg hover:bg-green-600">
                                                                    <?php echo lang('website.see_your_order_details'); ?> 
                                                                </a>
                                                            </div>
                                                        </div>
                                                    `;

                            const popupContainer = document.getElementById('orderResponsePopup');
                            popupContainer.innerHTML = popupContent;
                            popupContainer.classList.remove('hidden');
                            showToast(captureData.message, "success");

                        });
                },
                onCancel: function(data) {
                    showToast('Payment cancelled!', "error");
                },
                onError: function(err) {
                    console.error('Error:', err);
                },
                style: {
                    layout: 'horizontal',
                    color: 'gold',
                    shape: 'rect',
                    label: 'pay',
                    tagline: false
                },
                message: {
                    amount: 100,
                    align: 'center',
                    color: 'black',
                    position: 'top',
                }
            }).render('#paypal-button-container'); // Render the PayPal button
        </script>
<?php endif;
endforeach; ?>
