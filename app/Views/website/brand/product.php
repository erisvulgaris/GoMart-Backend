        ...
        <section class="mt-2 md:mt-4 md:container md:mx-auto px-3">
            <div class="row bg-white dark:bg-gray-800 mb-2 p-4 rounded-lg">
                <div class="flex justify-between">
                    <h2 class="text-lg font-medium z-10 dark:text-white"><?= $brand ?></h2>
                </div>
            </div>
        </section>
        ...
        <section class="mt-2 md:mt-4 md:container md:mx-auto px-3">
            <div class="grid lg:grid-cols-6 md:grid-cols-4 grid-cols-2 gap-4">
                <?php foreach ($products as $product): ?>
                    ...
                    <div class="rounded-lg bg-white dark:bg-gray-800 border border-green-500 dark:border-green-700" id="<?= $product['slug'] ?>">
                        <div class="flex-auto p-2">
                            ...
                            <div class="flex flex-col gap-1">
                                <h3 class="text-sm truncate font-semibold dark:text-white"><a href="shop-single.html"><?= $product['product_name'] ?></a></h3>
                                <span class="text-xs text-gray-500 dark:text-gray-400"><?php echo $firstVarient['title'] ?></span>
                                <span class="productDeliveryTime text-xs text-green-600 dark:text-green-400 font-medium hidden"><i class="fi fi-rr-clock"></i> <span class="productDeliveryTimeVal"></span> min delivery</span>
                                <div class="flex justify-between items-center mt-2">
                                    <div class="flex flex-col">
                                        <?php if ($firstVarient['discounted_price'] > 0): ?>
                                            <span class="text-sm text-gray-900 dark:text-gray-100 font-semibold"><?= $firstVarient['discounted_price'] ?></span>
                                            <span class="line-through text-gray-500 dark:text-gray-400 text-xs"><?= $firstVarient['price'] ?></span>
                                        <?php else: ?>
                                            <span class="text-sm text-gray-900 dark:text-gray-100 font-semibold"><?= $firstVarient['price'] ?></span>
                                        <?php endif; ?>
                                    </div>
