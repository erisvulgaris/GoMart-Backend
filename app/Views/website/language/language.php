        ...
        <section class="mt-2 md:mt-4 md:container md:mx-auto px-3">
            <div class="row bg-white dark:bg-gray-800 mb-2 p-4 rounded-lg">
                <div class="flex justify-between">
                    <h2 class="text-lg font-medium z-10 dark:text-white"><?php echo lang('website.language'); ?></h2>
                </div>
            </div>
        </section>
                    ...
                    <div class="overflow-x-auto mt-2">
                        <table class="min-w-full table-auto bg-white dark:bg-gray-800 rounded-lg shadow-md">
                            <thead class="border-b dark:border-gray-700">
                                <tr>
                                    <th class="px-4 py-2 text-left text-base font-medium w-auto dark:text-white"><?php echo lang('website.language'); ?></th>
                                    <th class="px-4 py-2 text-left text-base font-medium w-auto dark:text-white"><?php echo lang('website.status'); ?></th>
                                    <th class="px-4 py-2 text-left text-base font-medium w-auto dark:text-white"><?php echo lang('website.action'); ?></th>
                                </tr>
                            </thead>
                                ...
                                <?php if (!empty($languageList) && is_array($languageList)): ?>
                                    <?php foreach ($languageList as $language): ?>
                                        <tr class="hover:bg-gray-100 dark:hover:bg-gray-700">
                                            <td class="px-4 py-2 text-sm w-auto dark:text-gray-300"><?= htmlspecialchars($language['language']) ?></td>
                                            <td class="px-4 py-2 text-sm w-auto">
                                                <?php if (session()->get('site_lang') == $language['lang_short']): ?>
                                                    <div class="inline-flex items-center gap-2 border border-blue-600 dark:border-blue-500 rounded-lg px-2 py-1">
                                                        <i class="fi fi-rr-check-circle text-blue-600 dark:text-blue-400"></i>
                                                        <span class="text-blue-600 dark:text-blue-400 text-sm font-medium"> <?php echo lang('website.default'); ?></span>
                                                    </div>
                                                <?php endif; ?>
                                            </td>
                                            <td class="px-4 py-2 text-sm w-auto">
                                                <a href="<?= base_url("language/".$language['id'])?>" class="inline-flex items-center gap-2 px-3 h-8 leading-8 rounded-lg bg-green-100 dark:bg-green-900/30 text-green-600 dark:text-green-400 whitespace-nowrap">
