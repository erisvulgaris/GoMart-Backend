...
class Order extends BaseController
{
    ...
    public function orderDetails($order_id)
    {
        ...
        // Fetch tax breakdowns for all order products
        $orderProductTaxModel = new OrderProductTaxModel();
        $allOrderProductIds = array_column($orderProducts, 'id');
    ...
    public function fetchLiveDeliveryTracking()
    {
        if (
            ...
            return $this->response->setJSON(['status' => 'error', 'message' => 'Not authorized'])->setStatusCode(401);
        }

        $data = $this->request->getJSON(true);

        if (empty($data['order_id'])) {
            return $this->response->setJSON(['status' => 'error', 'message' => 'Order ID is required'])->setStatusCode(400);
        }
        ...
        if (!$user) {
            return $this->response->setJSON(['status' => 'error', 'message' => 'User not found'])->setStatusCode(401);
        }

        $orderModel = new OrderModel();
        $order = $orderModel->where('id', $data['order_id'])->where('user_id', $user['id'])->first();
        ...
        $deliveryTracking = $deliveryTrackingModel->where('order_id', $data['order_id'])->first();

        if (!$deliveryTracking) {
            return $this->response->setJSON(['status' => 'error', 'message' => 'Delivery boy hasn\'t started delivery yet'])->setStatusCode(404);
        }
    ...
    /**
     * Calculate and save delivery charge taxes (always inclusive) into order_charge_taxes.
     * Called after every order placement if delivery_charge_tax_status is enabled.
     */
    ...
    /**
     * Calculate and save additional charge taxes (always inclusive) into order_charge_taxes.
     * Called after every order placement if additional_charge_tax_status is enabled.
     */
    ...
    public function placeCODOrder()
    {
        ...
        $orderData = [
            ...
            'delivery_instruction' => isset($data['deliveryInstruction']) ? $data['deliveryInstruction'] : '',
            'delivery_tip_amount' => isset($data['deliveryTip']) ? (float)$data['deliveryTip'] : 0,
            'billing_gst' => isset($data['gstNumber']) ? strtoupper(trim($data['gstNumber'])) : ''
        ];
        ...
        if ($orderModel->insert($orderData)) {
                ...
                if ($product && $variant) {
                    ...
                    if (!empty($productTaxes)) {
                        ...
                        if ($product['tax_included_in_price']) {
                            // Inclusive: extract tax from price for display, don't add to total
                            ...
                            // tax_amount = 0 for inclusive (already in price)
                            $taxAmount = 0;
                        } else {
                            // Exclusive: calculate tax to add on top
                            foreach ($productTaxes as $tax) {
            ...
            $this->sendOrderNotifications($orderId, $user['id']);
    ...
    public function createRazorpayOrder()
    {
        ...
        $deliveryTip = isset($data['deliveryTip']) ? (float)$data['deliveryTip'] : 0;

        $walletBalance = (float) $user['wallet'];
        $total = $subTotal + $taxTotal + $deliveryCharge + $additional_charge + $deliveryTip - $coupon_amount;
        ...
        $paymentAmount = (round($subTotal + $taxTotal + $deliveryCharge + $additional_charge + $deliveryTip - $coupon_amount - $walletApplied, 2)) * 100;
    ...
    public function verifyRazorpayPayment()
    {
        ...
        try {
            ...
            if (hash_equals($generatedSignature, $razorpaySignature)) {
                ...
                $placedOrderId = $data['order_id'];

                $orderStatusesModel = new OrderStatusesModel();
                $data = [
                    'orders_id' => $placedOrderId,
                    'status' => 2,
                    'created_by' => $user['id'],
                    'user_type' => 'Customer',
                    'created_at' => date('Y-m-d H:i:s'), // Use the current timestamp
                ...
                $this->sendOrderNotifications($placedOrderId, $user['id']);
    ...
    public function createPaypalOrder()
    {
        ...
        $walletBalance = (float) $user['wallet'];
        $total = round($subTotal + $taxTotal + $deliveryCharge + $additional_charge + $deliveryTip - $coupon_amount, 2);
    ...
    public function capturePaypalOrder()
    {
        ...
        if ($httpCode === 201) {
            ...
            $this->sendOrderNotifications($order['id'], $user['id']);
    ...
    public function verifyPaystackOrder()
    {
        ...
        $total = $order['subtotal'] + $order['tax'] + $additional_charge + $order['delivery_charge'] + (float)($order['delivery_tip_amount'] ?? 0) - $order['coupon_amount'] - $order['used_wallet_amount'];
        ...
        if (round($amount, 2) == round($total, 2)) {
            ...
            $this->sendOrderNotifications($order_id, $user['id']);
    ...
    private function sendOrderNotifications(int $orderId, int $userId): void
    {
        ...
        // Collect seller IDs from the order products
        $orderProducts = $orderProductModel->select('seller_id')->where('order_id', $orderId)->findAll();
        $sellerIds = array_unique(array_column($orderProducts, 'seller_id'));
