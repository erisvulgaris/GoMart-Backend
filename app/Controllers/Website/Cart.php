...
class Cart extends BaseController
{
    public function addToCart()
    {
        ...
        return $this->response->setJSON([
            ...
            'itemImages' => $this->getCartItemImages($cartItems),
            'subtotal' => $subTotal,
        ...
        foreach ($cartItems as $cartItem) {
            ...
            if ($product['tax_id']) {
                ...
                if(!isset($tax['percentage'])){
                    ...
                    $taxTotal += ($cartItem['quantity'] * $variant['discounted_price']) * $tax['percentage'] / 100;
                }
            }
        }
                ...
                if (!empty($sellerIds)) {
                    ...
                    $sellerItems = array_map(function ($seller) use ($cartsModel, $cartCondition) {
                        ...
                        return [
                            ...
                            'logo' => $seller['logo'] ? base_url($seller['logo']) : '',
    ...
    public function fetchCartItemCount()
    {
        ...
        if (session()->has('email') || session()->has('mobile')) {
            ...
            $cartItems = $cartsModel->where('guest_id', $guestId)->findAll();
            $cartItemCount = count($cartItems);
        }
        ...
        return $this->response->setJSON([
            'status' => 'success',
            'itemCount' => $cartItemCount,
            'itemImages' => isset($cartItems) ? $this->getCartItemImages($cartItems) : [],
        ]);
    }

    private function getCartItemImages($cartItems, int $limit = 3): array
    {
        $productModel = new ProductModel();
        $images = [];
        $seen = [];
        foreach ($cartItems as $item) {
            ...
            $seen[$pid] = true;
            $product = $productModel->select('main_img')->find($pid);
            if ($product && !empty($product['main_img'])) {
                $images[] = base_url($product['main_img']);
            }
