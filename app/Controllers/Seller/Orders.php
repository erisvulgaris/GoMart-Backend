...
class Orders extends BaseController
{
    public function index()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            return view('sellerPanel/orders/orders', $data);
        } else {
    ...
    public function list()
    {
        ...
        $builder = $orderModel->select(
            'orders.id as order_id, orders.user_id, orders.additional_charge, orders.address_id, orders.subtotal, orders.tax, orders.used_wallet_amount, orders.delivery_charge, orders.coupon_amount,  orders.order_date, orders.delivery_date, orders.timeslot, orders.delivery_boy_id, orders.transaction_id, orders.status, GROUP_CONCAT(DISTINCT seller.store_name) as seller_names, user.name as user_name, user.mobile as user_mobile, address.address, address.city_id, address.area, address.city, address.state, address.pincode, delivery_boy.name as delivery_boy_name, delivery_boy.mobile as delivery_boy_mobile, order_status_lists.status as order_status, order_status_lists.color as order_status_color'
        )
        ...
        END) as subtotal')
            ...
            if ($sellerInfo['view_customer_details'] == 1) {
                $output['data'][] = [
                    ...
                    "<a data-tooltip='tooltip' target='_blank' title='View Order' href='" . base_url("seller/orders/view/{$order['order_id']}") . "' class='btn btn-primary-light  btn-xs'>
                ...
                $output['data'][] = [
                    ...
                    "<a data-tooltip='tooltip' target='_blank' title='View Order' href='" . base_url("seller/orders/view/{$order['order_id']}") . "' class='btn btn-primary-light  btn-xs'>
                        <i class='fi fi-tr-magnifying-glass-eye'></i></a>
                       <a type='button' data-tooltip='tooltip' title='Delete Order' onclick='deleteOrder({$order['order_id']})' class='btn btn-danger btn-xs'>
    ...
    public function view($order_id)
    {
        ...
        END) as subtotal')
                ...
                ->first();
            $data['sellerInfo'] = $sellerModel->select('view_customer_details')->where('id', session()->get('user_id'))->where('is_delete', 0)->where('status', 1)->first();
            return view('sellerPanel/orders/orderDetails', $data);
        } else {
            return redirect()->to('seller/auth/login');
        }
    }
    public function downloadInvoice()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            $data['country'] = $countryModel->where('is_active', 1)->first();
            $data['orderDetails'] = $orderModel->select(
                'orders.id as order_id, orders.user_id, orders.address_id, orders.subtotal, orders.tax, orders.used_wallet_amount, 
        ...
        END) as subtotal')
            ...
            $html = view('sellerPanel/orders/invoice',  $data);
            $html = "
                    <html>
                        <head>
    ...
    public function updateOrderStatus()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            if ($status == 7) {
                if ($appSetting['seller_can_cancel_order'] != 1) {
                    return $this->response->setJSON(['success' => false, 'message' => 'Permission denied for cancel order']);
                }
            }

            $orderProductModel = new OrderProductModel();
            $multipleSellerIsFound = $orderProductModel->where('seller_id !=', session()->get('user_id'))->where('order_id', $order_id)->first();

            if ($multipleSellerIsFound['id']) {
                return $this->response->setJSON(['success' => false, 'message' => 'Only Admin can update Order Staus']);
            }


            if ($status == 6) {
                if ($appSetting['seller_can_complete_order'] != 1) {
                    return $this->response->setJSON(['success' => false, 'message' => 'Permission denied for completing order']);
                }
                    ...
                    if ($appSetting['delivery_boy_bonus_setting'] == 1) {
                        ...
                        if ($deliveryBoy['bonus_type'] == 1) {
                            ...
                            if ($selectOrderDetails['payment_method_id'] == 1) {
                                ...
                                $totalCashCollectionAmount = $deliveryBoy['cash_collection_amount'] + $selectOrderDetails['subotal'] + $selectOrderDetails['tax'] + $selectOrderDetails['delivery_charge'] + $selectOrderDetails['additional_charge'] - $selectOrderDetails['used_wallet_amount'];
                                ...
                                $deliveryBoyTransactionData = [
                                    ...
                                    'delivery_boy_id' => $deliveryBoy['id'],
                                    'type' => 'credit',
                                    'amount' => $selectOrderDetails['subotal'] + $selectOrderDetails['tax'] + $selectOrderDetails['delivery_charge'] + $selectOrderDetails['additional_charge'] - $selectOrderDetails['used_wallet_amount'],
    ...
    public function assignDeliveryBoy()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Seller') {
            ...
            $updateOrder = $orderModel->set($data)->where('id', $order_id)->update();
            if ($updateOrder) {
                $userDetails = $orderModel->select('user_id')->where('id', $order_id)->first();
                $deviceTokenModel = new DeviceTokenModel();
                $dataForNotification = [
                    'screen' => 'Notification',
                ];
