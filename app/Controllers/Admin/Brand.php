...
class Brand extends BaseController
{
    ...
    public function add()
    {
        ...
        $brandModelCheck = new BrandModel();
        if ($brandModelCheck
            ->where('LOWER(brand)', strtolower($brand_name))
            ->countAllResults() > 0
        ) {
            $output['message'] = 'Brand name already exists.';
            return $this->response->setJSON($output);
        }
    ...
    public function update()
    {
        ...
        // Check for duplicate brand name (case-insensitive, exclude current record)
        $brandModelCheck = new BrandModel();
        if ($brandModelCheck->where('LOWER(brand)', strtolower($brand_name))->where('id !=', $brand_id)->countAllResults() > 0) {
            $output['message'] = 'Brand name already exists.';
            return $this->response->setJSON($output);
        }
