    ...
    public function list()
    {
        ...
        foreach ($users as $user) {
            ...
            if (!empty($email) && strpos($email, '@') !== false) {
                $parts = explode('@', $email);
                $hiddenEmail = substr($parts[0], 0, 2) . str_repeat('*', 8) . '@' . $parts[1];
                $mailto = "<a class='text-dark' href='mailto:$email'>$hiddenEmail</a>";
            } else {
                $mailto = ''; // Email is empty or invalid — show nothing
            }
            $mobile = $user['mobile'];
            $mobileLength = strlen($mobile);
            
            if ($mobileLength >= 8) {
                $maskedMobile = substr($mobile, 0, 2) .
                                str_repeat('*', $mobileLength - 6) .
                                substr($mobile, -4);
            } else {
                // Always show 10 asterisks for short numbers
                $maskedMobile = str_repeat('*', 10);
            }
            
            $tel = "<a class='text-dark' href='tel:$mobile'>$maskedMobile</a>";
            $action = "
