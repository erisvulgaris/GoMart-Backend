...
class Dashboard extends BaseController
{
    public function index()
    {
        ...
        if ($session->has('user_id')  && session('account_type') == 'Admin') {
            ...
            // New registrations & pending items
            $newSellerCount = $sellerModel->where('status', 0)->where('is_delete', 0)->countAllResults();
            $newDeliveryBoyCount = $deliveryBoyModel->where('status', 0)->where('is_delete', 0)->countAllResults();
            $newOrderCount = $orderModel->where('status', 1)->where('is_pos_order', 0)->countAllResults();
            $pendingRatingsCount = $productRatingsModel->where('is_approved_to_show', 0)->countAllResults();
            ...
            $data = [
                ...
                'newDeliveryBoyCount' => $newDeliveryBoyCount,
