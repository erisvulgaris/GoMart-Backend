...
class Pos extends BaseController
{
    ...
    public function searchProducts()
    {
        ...
        if (!$sellerId || !$keyword) {
            return $this->response->setJSON([
                'success' => false,
                'message' => 'Seller ID and keyword are required'
            ]);
        }
    ...
    public function saveCartSession()
    {
        ...
        $data = [
            ...
            'created_by_admin' => session()->get('user_id') ?? 0  
        ];
    ...
    public function getCartSessions()
    {
        ...
        $sessions = $this->posCartSessionModel
            ...
            ->where('created_by_admin', session()->get('user_id'))
    ...
    public function printInvoice()
    {
        ...
        $order = $this->orderModel
            ->select('orders.*, pos_payment_method.name as pos_payment_method_name, payment_method.title as online_payment_method_name')
    ...
    public function deleteCartSession()
    {
        ...
        $this->posCartSessionModel->where('created_by_admin', session()->get('user_id'))->where('session_id', $sessionId)->delete();
    ...
    public function reportIndex()
    {
        ...
        if ($session->has('user_id') && session('account_type') == 'Admin') {
            ...
            return view('report/posReport', $data);
        } else {
            return redirect()->to('admin/auth/login');
        }
    }
    ...
    public function reportList()
    {
        ...
        // Apply seller filter
        if (!empty($sellerId)) {
            $builder->where('order_products.seller_id', $sellerId);
        }
        ...
        foreach ($orders as $order) {
            ...
            $action = '<a href="' . base_url('admin/pos/printInvoice?order_id=' . $order['id']) . '" target="_blank" class="btn btn-primary-light btn-xs" data-tooltip="tooltip" title="Print Invoice"><i class="fi fi-tr-print"></i></a>';
