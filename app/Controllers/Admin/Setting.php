...
use App\Models\AdditionalChargeTaxModel;
...
class Setting extends BaseController
{
    ...
    function storeIndex()
    {
        ...
        $data['allTaxes'] = $taxModel->where('is_active', 1)->where('is_delete', 0)->findAll();

        $dctModel = new DeliveryChargeTaxModel();
        $activeDct = $dctModel->findAll();
        $data['selectedDeliveryTaxIds'] = array_column($activeDct, 'tax_id');

        $actModel = new AdditionalChargeTaxModel();
        $activeAct = $actModel->findAll();
        $data['selectedAdditionalTaxIds'] = array_column($activeAct, 'tax_id');
    ...
    public function updateSetting()
    {
        ...
        // Handle delivery charge tax IDs separately (only when submitted by orderForm)
        if (array_key_exists('delivery_charge_tax_ids', $postData)) {
            $deliveryChargeTaxIds = $postData['delivery_charge_tax_ids'] ?? [];
            $dctModel = new DeliveryChargeTaxModel();
            $dctModel->syncTaxes(array_map('intval', (array) $deliveryChargeTaxIds));
            unset($postData['delivery_charge_tax_ids']);
        }

        // Handle additional charge tax IDs separately (only when submitted by orderForm)
        if (array_key_exists('additional_charge_tax_ids', $postData)) {
            $additionalChargeTaxIds = $postData['additional_charge_tax_ids'] ?? [];
            $actModel = new AdditionalChargeTaxModel();
            $actModel->syncTaxes(array_map('intval', (array) $additionalChargeTaxIds));
            unset($postData['additional_charge_tax_ids']);
        }
    ...
    function sellerAppPolicy()
    {
        if (!session()->has('user_id') || session('account_type') != 'Admin') {
            return redirect()->to('admin/login');
        }
        if (!can_view('seller-app-policy')) {
            $output = ['success' => false, "message" => "Permission not allowed"];
            return $this->response->setJSON($output);
        }
        $settingModel = new SettingsModel();
        $data['settings'] = $settingModel->getSettings();
        return view('/sellerPolicy', $data);
    }

    public function updateSellerAppPolicy()
    {
        $output = [
            'success' => false,
            'message' => ''
    ...
    public function sysActivate()
    {
        ...
        if (!$inputRaw) {
            return $this->response->setJSON(['success' => false, 'message' => 'Purchase code is required.']);
        }
        ...
        if (!$resp || $httpCode !== 200) {
            return $this->response->setJSON(['success' => false, 'message' => 'Could not reach activation server. Try again.']);
        }
        ...
        return $this->response->setJSON(['success' => true, 'message' => 'Activated successfully.']);
    }
}
