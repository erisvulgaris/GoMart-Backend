...
class Subcategory extends BaseController
{
    ...
    public function add()
    {
        ...
        if ($this->validate([
            ...
            $subcategoryModelCheck = new SubcategoryModel();
            if ($subcategoryModelCheck->where('LOWER(name)', strtolower($sub_cat_name))->where('category_id', $cat_id)->countAllResults() > 0) {
                return $this->response->setJSON(['success' => false, 'message' => 'Subcategory name already exists in this category.']);
            }
    ...
    public function update()
    {
        ...
        // Check for duplicate subcategory name within the same category (case-insensitive, exclude current record)
        $subcategoryModelCheck = new SubcategoryModel();
        if ($subcategoryModelCheck->where('LOWER(name)', strtolower($sub_cat_name))->where('category_id', $cat_id)->where('id !=', $sub_cat_id)->countAllResults() > 0) {
            $output['message'] = 'Subcategory name already exists in this category.';
            return $this->response->setJSON($output);
        }
    ...
    public function getSub()
    {
        ...
        $categoryIDs = $this->request->getPost('cat_change');

        if (!empty($categoryIDs) && is_array($categoryIDs)) {
            $subcategoryModel = new SubcategoryModel();
            $sellerModel = new SellerModel();
            
            $categoryIds = $this->request->getPost('cat_change'); // this is now an array
            
            // ---------------- Subcategories (multiple category support) ----------------
            $data['subcategory'] = $subcategoryModel
                ->select('id, name, category_id')
                ->whereIn('category_id', $categoryIds)
                ->findAll();
            
            // ---------------- Sellers (multiple category support) ----------------
            $data['seller'] = $sellerModel
                ->select('seller.id, seller.store_name')
                ->join('seller_categories', 'seller_categories.seller_id = seller.id', 'left')
                ->whereIn('seller_categories.category_id', $categoryIds)
                ->groupBy('seller.id')  // avoid duplicates if seller has multiple categories
                ->findAll();
