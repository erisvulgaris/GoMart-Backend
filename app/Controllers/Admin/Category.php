...
class Category extends BaseController
{
    ...
    public function add()
    {
        ...
        $categoryModelCheck = new CategoryModel();
        if ($categoryModelCheck
            ->where('LOWER(category_name)', strtolower($cat_name))
            ->countAllResults() > 0
        ) {
    ...
    public function update()
    {
        ...
        // Check for duplicate category name (case-insensitive, exclude current record)
        $categoryModelCheck = new CategoryModel();
        if ($categoryModelCheck->where('LOWER(category_name)', strtolower($cat_name))->where('id !=', $cat_id)->countAllResults() > 0) {
            $output['message'] = 'Category name already exists.';
            return $this->response->setJSON($output);
        }
