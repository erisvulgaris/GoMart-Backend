...
// 4. Ensure a seller exists assigned to deliverable_area_id = 1 (Gorakhpur)
...
foreach ($products_data as $p) {
    ...
    $db->query("INSERT INTO `product_categories` (`product_id`, `category_id`) VALUES ({$p['id']}, $cat_id) ON DUPLICATE KEY UPDATE category_id=$cat_id");
    ...
    $db->query("INSERT INTO `product_variants` (`product_id`, `status`, `title`, `price`, `discounted_price`, `stock`, `is_unlimited_stock`, `stock_unit_id`, `is_delete`) 
                VALUES ({$p['id']}, 1, '{$p['variant_title']}', {$p['price']}, {$p['disc_price']}, 100, 0, 0, 0)
