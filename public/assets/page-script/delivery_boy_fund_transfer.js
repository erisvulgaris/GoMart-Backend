$(document).ready(function() {
    ...
    var table = $("#view_fund_transfer").DataTable({
        paging: true,
        lengthChange: false,
        ordering: true,
        info: true,
        ...
        responsive: true,
        ajax: {
            url: "/admin/delivery_boy/fund_transfer/list",
            type: "POST",
            dataType: "json",
            dataSrc: "data",
            data: function(d) {
    ...
    $("#fundTransferForm").on('submit', function(e) {
        ...
        $.ajax({
            url: '/admin/delivery_boy/fund_transfer/add', 
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
                if (response.success) {
                    ...
                    selectDeliveryBoy.append('<option value="">Select Delivery Boy</option>');
                    response.deliveryBoy.forEach(boy => {
                        const option = `<option value="${boy.id}">${boy.name} (${response.currency} ${boy.balance})</option>`;
