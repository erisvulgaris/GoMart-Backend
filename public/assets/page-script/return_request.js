...
$(document).ready(function() {
    ...
    $("#updateRequestForm").on('submit', function(e) {
        ...
        $.ajax({
            url: '/admin/return-request/update',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $("#updateReturnedForm").on('submit', function(e) {
        e.preventDefault();
        var formData = new FormData($("#updateReturnedForm")[0]);;

        $.ajax({
            url: '/admin/update-returned-status/update',
            type: 'POST',
            data: formData,
            contentType: false,
            processData: false,
            success: function(response) {
                if (response.success) {
                    toastr.success(response.message, "Admin says");
                    $("#updateReturnedForm").trigger('reset')
                    table.ajax.reload();
    ...
    $.ajax({
        ...
        success: function(response) {
            if (response.success == true) {
                ...
                $(`#delivery_boy_id option`).prop('selected', false);
                $(`#delivery_boy_id option[value="${response.response.delivery_boy_id}"]`).prop('selected', true);
                $(`#update_status option`).prop('selected', false);
                $(`#update_status option[value="${response.response.status}"]`).prop('selected', true);
...
function updateToReturned(id) {
    $("#modal-default1").modal('show')
    $.ajax({
        url: "/admin/update-returned-status/view",
        type: "POST",
        data: {
            return_request: id
        },
        ...
        success: function(response) {
            if (response.success == true) {
                $(`#return_request_id`).val(response.response.id);
                $(`#update_returned_status option`).prop('selected', false);
                $(`#update_returned_status option[value="${response.response.update_returned_status}"]`).prop('selected', true);
            } else {
                toastr.error(response.message, "Admin says");
                $("#modal-default1").modal('hide')
                $(`#updateReturnedForm`).trigger('reset');
