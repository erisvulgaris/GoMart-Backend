...
mysql -h"db" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" --ssl=0 -D"$MYSQL_DATABASE" -e "UPDATE settings SET value='AIzaSyAmX29-nyb3BDTtovxvhaJR_u82fphs-6M' WHERE name='map_api_key';"
mysql -h"db" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" --ssl=0 -D"$MYSQL_DATABASE" -e "UPDATE settings SET value='https://cityloopapp.com' WHERE name='website';"
mysql -h"db" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" --ssl=0 -D"$MYSQL_DATABASE" -e "UPDATE settings SET value='CityLoop' WHERE name='business_name';"
mysql -h"db" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" --ssl=0 -D"$MYSQL_DATABASE" -e "UPDATE settings SET value='info@cityloopapp.com' WHERE name='email';"
mysql -h"db" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" --ssl=0 -D"$MYSQL_DATABASE" -e "UPDATE settings SET value='{\"name\":\"CityLoop\",\"host\":\"smtp.hostinger.com\",\"username\":\"YOUR_SMTP_USERNAME\",\"password\":\"YOUR_SMTP_PASSWORD\",\"port\":\"465\",\"encryption\":\"ssl\"}' WHERE name='mail_config';"
mysql -h"db" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" --ssl=0 -D"$MYSQL_DATABASE" -e "UPDATE settings SET value='    <div><h1>About Us</h1>    <p>Welcome to <strong>CityLoop</strong>, your premium hyperlocal quick-commerce platform. We deliver fresh groceries and daily essentials to your doorstep in minutes.</p>    <h2>Our Mission</h2>    <p>We aim to simplify shopping with a user-friendly experience, secure payments, and reliable deliveries.</p>    <p><strong>Email:</strong> info@cityloopapp.com</p><p>Thank you for choosing <b>CityLoop</b> 🚀</p></div>' WHERE name='customer_app_about';"
mysql -h"db" -u"$MYSQL_USER" -p"$MYSQL_PASSWORD" --ssl=0 -D"$MYSQL_DATABASE" -e "UPDATE settings SET value='<h3><strong>About CityLoop Delivery App</strong></h3><p>...</p>' WHERE name='delivery_app_about';"
